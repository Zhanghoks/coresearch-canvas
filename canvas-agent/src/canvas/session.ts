...
import { ResearchArtifactStore, type ResearchArtifactInput } from "../artifacts/store.js";
import { logger } from "../utils/logger.js";
import { buildCanvasToolRequest, fitAttachmentNodeSize } from "./operations.js";
import { advanceResearchWorkflow, type ResearchWorkflowIntent } from "./research-workflow.js";
...
const SITE_TOOLS = new Set<ToolName>(["site_navigate", "canvas_list_projects", "assets_list", "assets_add", "generation_get_status"]);
...
export class CanvasSession {
    ...
    constructor(activeThreadId = "", private readonly artifactStore?: ResearchArtifactStore) {
    ...
    /** 会话创建结束后提交线程。无 MCP 清单时直接 ready；有 MCP 状态时沿用原预热判定。 */
    completeConversationPreparation(threadId: string) {
        ...
        if (items.length === 0) {
            this.conversationInventoryComplete = true;
            return this.updateConversation({ threadId, status: "ready", error: undefined });
        }
        const hasPending = !this.conversationInventoryComplete || items.some((item) => item.status === "starting");
        const requiredFailure = statuses["infinite-canvas"]?.status !== "ready";
        const hasFailure = items.some((item) => item.status === "failed" || item.status === "cancelled");
        ...
        return this.updateConversation({
            ...
            error: requiredFailure ? `CoResearch MCP 初始化失败${requiredFailureDetail ? `：${requiredFailureDetail}` : ""}` : undefined,
        });
    }
    ...
    /** 只按发起网页当前绑定的 Project 列出研究产物。 */
    async listArtifactsForClient(clientId: string) {
        const { store, userId, projectId } = this.artifactStoreForClient(clientId);
        return await store.list(userId, projectId);
    }

    /** 只按发起网页当前绑定的 Project 读取研究产物正文。 */
    async readArtifactForClient(clientId: string, artifactId: string) {
        const { store, userId, projectId } = this.artifactStoreForClient(clientId);
        return await store.read(userId, projectId, artifactId);
    }
    ...
    async callTool(name: unknown, rawInput: unknown) {
        ...
        if (name === "research_artifact_write" || name === "research_artifact_list" || name === "research_artifact_read") {
            const { store, userId, projectId } = this.artifactScope();
            if (name === "research_artifact_list") return await store.list(userId, projectId);
            if (name === "research_artifact_read") return await store.read(userId, projectId, String(input.artifactId));
            const conversationId = this.conversationState.conversationId;
            const turnId = this.codexState.turnId;
            if (!conversationId || !turnId) throw new Error("Artifact 写入必须发生在一个有效 Agent turn 中");
            const artifact = await store.write({ userId, projectId, conversationId, turnId }, input as unknown as ResearchArtifactInput);
            this.emitAll("artifacts_changed", { projectId, artifactId: artifact.id });
            return artifact;
        }
        if (name === "research_workflow_advance") {
            if (!this.canvasState) throw new Error("当前没有已连接画布");
            const workflow = advanceResearchWorkflow(this.canvasState, input as unknown as ResearchWorkflowIntent);
            if (!workflow.ops.length) return workflow;
            const canvasResult = await this.requestCanvasTool("canvas_apply_ops", {
                ops: workflow.ops,
            });
            return { ...workflow, canvasResult };
        }
    ...
    private artifactStoreForClient(clientId: string) {
        if (!clientId || !this.clients.has(clientId)) throw new Error("当前网页未连接");
        const snapshot = this.canvasStates.get(clientId);
        const projectId = snapshot?.projectId;
        const userId = snapshot?.ownerUserId;
        if (!projectId || !userId) throw new Error("当前画布缺少 Project 身份");
        if (!this.artifactStore) throw new Error("Artifact Store 未配置");
        return { store: this.artifactStore, userId, projectId };
    }

    private artifactScope() {
        const userId = this.canvasState?.ownerUserId;
        const projectId = this.canvasState?.projectId;
        if (!userId || !projectId) throw new Error("当前画布缺少 Project 身份");
        if (!this.artifactStore) throw new Error("Artifact Store 未配置");
        return { store: this.artifactStore, userId, projectId };
    }
