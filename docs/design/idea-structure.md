...
**Idea 是一个持续演化的研究状态，包含一组可独立追问、验证、替换的 Sections，以及用户选定的表述、尚未采纳的候选、文献判断与决策历史。** 十个维度是组织内容的骨架，不是必须按顺序填完的问卷。
...
每个 Section 独立维护 Current、Evidence、Proposals 和 History。Section 被替换时不重写整个 Idea；如果它被其他 Section 依赖，则产生 dependency check，等待用户决定是否同步修订相关 Section。
