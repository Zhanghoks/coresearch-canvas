# CoResearch Method Exploration / Candidate 设计

- 日期：2026-09-18
- 状态：设计提案；本次不实现页面、Skill、服务、方法代码或真实文献检索。
- 上游：Research Question Formation、Evaluation Exploration；Hypothesis / Problem 作为可选设计约束。
- 关联：[Problem Formation 设计层](./04-problem-formation.md)、[Deep Dive Research Map](./02-research-landscape.md)、[Idea 结构设计](../idea-structure.md)。
- 下游：[Research Design Co-Design](./07-research-design.md)。

## 当前对齐契约

本文的 Method requirements、Method Families、closest-work、feasibility 和 simplicity 内容继续有效，但产品顺序改为：Evaluation Strategy 确认后才进入 Method Exploration。Approach 是总体检验策略，Method 是实现候选；二者都不能在 Evaluation Targets 之前锁定。每个 Method Candidate 必须连接至少一个 Evaluation Strategy，并保留已有 Result、Limitation、Supporting Literature 与 proposed difference。
- 产品流程真值：[Research Flow](../research-flow.md)。本文提供 Method Exploration 和 Method Candidate 材料；Approach 只作为 Research Design 内的总体检验策略，方法组成与 Evaluation 对齐归 [07-research-design.md](./07-research-design.md)。
...
```text
Selected Direction  当前研究空间
Research Question  用户决定继续追问的问题
Evaluation Strategy  什么结果才算支持或反驳
Problem / Hypothesis  可选的设计约束与机制解释
```
