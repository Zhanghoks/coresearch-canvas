...
export function AgentChatPromptInput({ value, disabled, placeholder, theme, onChange, onSubmit, onAddFiles }: {
    ...
    const [hasDraft, setHasDraft] = useState(() => Boolean(value.trim()));
    ...
    useEffect(() => {
        ...
        setHasDraft(editorHasContent(editor));
    }, [theme, tokens, value]);
    ...
    const syncFromEditor = () => {
        ...
        if (!editor) return;
        setHasDraft(editorHasContent(editor));
        const previous = useAgentStore.getState().prompt;
        const next = serializeEditor(editor);
        emit(next);
        syncSelectedMetadata(editor, previous);
        syncCommand();
    };
    ...
    return (
        <div ref={containerRef} className="relative group">
            {!value.trim() && !hasDraft ? <div className="pointer-events-none absolute left-1 top-1 text-sm leading-6 group-focus-within:hidden" style={{ color: theme.node.placeholder }}>{placeholder}</div> : null}
            <div
                ref={editorRef}
                ...
                onInput={() => {
                    setHasDraft(editorHasContent(editorRef.current));
...
function syncSelectedMetadata(editor: HTMLElement, previousPrompt: string) {
    const state = useAgentStore.getState();
    const text = serializeEditor(editor);
    const references = state.canvasReferences.filter((item) => {
        const marker = agentReferenceMarker(item);
        if (text.includes(marker)) return true;
        return !previousPrompt.includes(marker);
    });
...
function editorHasContent(editor: HTMLElement | null) {
    if (!editor) return false;
    if (editor.querySelector("[data-agent-token]")) return true;
    return Boolean((editor.textContent || "").replace(/[\u200b\ufeff]/g, "").trim());
}
