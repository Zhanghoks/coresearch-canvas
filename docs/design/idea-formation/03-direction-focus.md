# CoResearch Step 2：Direction Exploration 与 Direction Selection

- 日期：2026-09-17
- 状态：设计提案；本次不实现页面、Skill 或服务。
- 上游：[Step 1 · Seed Framing](./01-seed.md)。
- 关联：[Step 1 · Seed](./01-seed.md)、[Idea 结构设计](../idea-structure.md)。
- 下游：[Step 3 · Direction Deep Dive](./02-research-landscape.md)。
- 产品流程真值：[Research Flow](../research-flow.md)。候选先活在 Conversation，用户保存进 Canvas 后才成为 Direction；结束条件是至少一个 `selected_for_deep_dive`，不是确认最终 Focus。若与 Flow 冲突，以 Flow 为准。

## 当前对齐契约

本步骤是 Seed 之后的广探和导航，不读取一个预先完成的全领域 Landscape。目标是以 Direction Coverage 为停止依据，允许用户保存多个 Direction，并通过 Direction Polish 重搜接近区域。`Focus` 不再是独立的产品实体；当前长期实体是 Direction，用户明确选择后进入 Deep Dive。Research Question 只能在 Deep Dive 的 Paper Section 证据之后形成。
...
Confirmed Seed
      ↓
Direction Coverage / Candidate Map
      ↓
保存、比较或 Polish Direction candidates
      ↓
用户明确选择 Selected Direction
      ↓
Step 3 · Direction Deep Dive
```
