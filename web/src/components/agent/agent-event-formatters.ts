...
/** 对话里只保留用户原话和 @引用；nodeId 说明只留给模型。 */
export function presentCanvasReferenceMessage(text: string, references: Array<{ nodeId: string; label: string; title: string; kind: "image" | "video" | "audio" | "text"; previewUrl?: string; text?: string }>) {
    const index = text.indexOf(CANVAS_REFERENCE_MARKER);
    if (index < 0) return { text, references };
    const visible = text.slice(0, index).trim().replace(/^请处理引用的画布素材。$/, "");
    if (references.length) return { text: visible, references };
    const parsed = [...text.slice(index).matchAll(CANVAS_REFERENCE_LINE)].map((match) => ({
        nodeId: match[2],
        label: match[1].replace(/^@/, ""),
        title: match[3],
        kind: match[4] as "image" | "video" | "audio" | "text",
    }));
    return { text: visible, references: parsed };
}

export function userTextWithCanvasReferences(text: string, references: Array<{ label: string }>, fallback: string) {
    const markers = references.map((item) => `@${item.label}`).filter((marker) => marker.length > 1 && !text.includes(marker));
...
/** 编辑某条用户消息时，应保留到其前一轮 turn（含）；首轮返回 null 表示清空后重建。 */
export function keepThroughTurnIdBeforeUserMessage(messages: AgentChatItem[], messageId: string) {
    const index = messages.findIndex((item) => item.id === messageId);
    if (index < 0) return null;
    const editedTurnId = messages[index].turnId || "";
    for (let i = index - 1; i >= 0; i -= 1) {
        const turnId = messages[i].turnId;
        if (turnId && turnId !== editedTurnId) return turnId;
    }
    return null;
}

/** 服务端回写用户消息时替换发送时的占位，不把它挪到本轮回复后面。 */
export function replaceSyntheticUserMessage(messages: AgentChatItem[], next: AgentChatItem) {
    const clientMessageId = next.clientMessageId || "";
    let index = messages.findIndex((item) => item.role === "user" && (item.id === next.id || (clientMessageId && item.clientMessageId === clientMessageId)));
    if (index < 0 && next.turnId) {
        index = messages.findIndex((item) => item.role === "user" && item.threadId === next.threadId && item.turnId === next.turnId);
    }
    if (index < 0) {
        for (let i = messages.length - 1; i >= 0; i -= 1) {
            const item = messages[i];
            if (item.role === "user" && !item.turnId && item.threadId === next.threadId && item.itemId === "synthetic:user") {
                index = i;
                break;
            }
        }
    }
    if (index < 0) return upsertAgentMessage(messages, next);
    const current = messages[index];
    const merged = { ...current, ...next, ...mergeLocalMessageMetadata(current, next) };
    return messages.map((item, itemIndex) => itemIndex === index ? merged : item);
}

function visibleUserText(text: string) {
    const index = text.indexOf(CANVAS_REFERENCE_MARKER);
    const visible = (index < 0 ? text : text.slice(0, index)).replace(/^请处理引用的画布素材。$/, "");
    return visible.replace(/@\S+/g, " ").replace(/\s+/g, " ").trim();
}

/** 同一句用户输入若被记成没有回复的另一轮，不再显示在本轮回复下面。 */
function collapseOrphanDuplicateUsers(messages: AgentChatItem[]) {
    const turnHasOutput = new Set<string>();
    messages.forEach((item) => {
        if (item.turnId && item.role !== "user") turnHasOutput.add(item.turnId);
    });
    const answered = new Set<string>();
    messages.forEach((item) => {
        if (item.role !== "user" || !item.turnId || !turnHasOutput.has(item.turnId)) return;
        const text = visibleUserText(item.text);
        if (text) answered.add(`${item.threadId || ""}\0${text}`);
    });
    return messages.filter((item) => {
        if (item.role !== "user" || !item.turnId || turnHasOutput.has(item.turnId)) return true;
        const text = visibleUserText(item.text);
        return !text || !answered.has(`${item.threadId || ""}\0${text}`);
    });
}

function collapseUnscopedDuplicates(messages: AgentChatItem[]) {
    return messages.filter((item) => {
        if (item.turnId || item.streamId || (item.role !== "assistant" && item.role !== "tool")) return true;
        const text = item.text.trim();
        return !messages.some((other) => {
            if (!other.turnId || other.role !== item.role) return false;
            const otherText = other.text.trim();
            if (text && otherText && (text === otherText || (otherText.length > 40 && text.startsWith(otherText.slice(0, 80))))) return true;
            const title = other.title || "";
            return !text && item.role === "tool" && Boolean(title && item.title?.includes(title));
        });
    });
}

export function hoistTurnUserMessages(messages: AgentChatItem[]) {
    const source = collapseOrphanDuplicateUsers(collapseUnscopedDuplicates(messages));
    const ordered: AgentChatItem[] = [];
    const turnStart = new Map<string, number>();
    source.forEach((item) => {
        const turnId = item.turnId || "";
        if (item.role === "user" && turnId && turnStart.has(turnId)) {
            const at = turnStart.get(turnId)!;
            ordered.splice(at, 0, item);
            turnStart.delete(turnId);
            turnStart.forEach((index, key) => {
                if (index >= at) turnStart.set(key, index + 1);
            });
            return;
        }
        if (turnId && item.role !== "user" && !turnStart.has(turnId)) turnStart.set(turnId, ordered.length);
        ordered.push(item);
    });
    return ordered;
}
...
export function mergeAgentMessages(snapshot: AgentChatItem[], current: AgentChatItem[], threadId: string, liveTurnKeys: ReadonlySet<string>) {
    ...
    current.filter((item) => item.threadId === threadId).forEach((item) => {
        ...
        if (index < 0) {
            const userIndex = matchingUserMessageIndex(messages, item, threadId, liveTurnKeys);
            if (userIndex >= 0) {
                const history = messages[userIndex];
                const metadata = mergeLocalMessageMetadata(history, item);
                const next = live && item.turnId ? { ...history, ...item, ...metadata } : { ...history, ...metadata };
                messages = messages.map((message, itemIndex) => itemIndex === userIndex ? next : message);
                return;
            }
...
function matchingUserMessageIndex(messages: AgentChatItem[], item: AgentChatItem, threadId: string, liveTurnKeys: ReadonlySet<string>) {
    if (item.role !== "user") return -1;
    return messages.findIndex((message) => {
        if (message.role !== "user") return false;
        if (item.clientMessageId && message.clientMessageId === item.clientMessageId) return true;
        if (item.turnId && message.turnId === item.turnId && message.threadId === threadId) return true;
        if (!item.turnId && message.threadId === threadId && message.itemId === "synthetic:user" && message.text === item.text && (!message.turnId || liveTurnKeys.has(`${threadId}\0${message.turnId}`))) return true;
        return false;
    });
}
...
export function normalizeHistoryMessages(messages: AgentChatItem[]) {
    return messages
        .filter((item) => (normalizeText(item.text) || item.role === "tool" || Boolean(item.canvasReferences?.length)) && item.itemId && item.threadId && item.turnId)
        .map(({ streamId: _streamId, ...item }) => scopeChatItem({ ...item, text: normalizeText(item.text) } as AgentChatItem, item.threadId!, item.turnId!));
}

const RUNNING_ACTIVITY = new Set(["inProgress", "in_progress", "running", "started", "pending"]);

/** 回合已停止时，把仍显示为进行中的思考和命令收成完成。 */
export function settleStoppedActivities(messages: AgentChatItem[], threadId: string, turnId = "") {
    return messages.map((message) => {
        if (threadId && message.threadId !== threadId) return message;
        if (turnId && message.turnId && message.turnId !== turnId) return message;
        const detail = message.detail && typeof message.detail === "object" ? message.detail as Record<string, unknown> : null;
        const running = detail ? RUNNING_ACTIVITY.has(String(detail.status || "")) : false;
        if (!running && !message.streamId) return message;
        return { ...message, streamId: undefined, ...(running && detail ? { detail: { ...detail, status: "completed" } } : {}) };
    });
}
