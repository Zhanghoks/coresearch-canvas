...
import { installBundledSkills, writeConfigFile, type CanvasAgentConfig } from "./config.js";
...
test("installBundledSkills installs managed reference files without overwriting user edits", () => {
    const base = makeTempBase();
    try {
        installBundledSkills(base);
        const reference = path.join(base, "research-workspace", "references", "artifact-contract.md");
        assert.match(fs.readFileSync(reference, "utf8"), /research_artifact_write/);
        fs.writeFileSync(reference, "user-owned\n");
        installBundledSkills(base);
        assert.equal(fs.readFileSync(reference, "utf8"), "user-owned\n");
    } finally {
        fs.rmSync(base, { recursive: true, force: true });
    }
});
