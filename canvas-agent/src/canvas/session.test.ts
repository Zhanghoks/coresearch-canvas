...
test("research workflow exploration returns a prompt without mutating the canvas", async (t) => {
    const session = new CanvasSession();
            ...
            nodes: [
                {
                    ...
                    width: 280,
                    height: 320,
                    metadata: { summary: "研究 AI Agent 与人的研究协作" },
                },
            ],
        },
        "first",
    ...
    const result = await session.callTool("research_workflow_advance", {
        kind: "explore",
        sourceNodeIds: ["seed-1"],
        action: "探索研究方向",
    });

    assert.match(String(field(result, "prompt")), /候选只出现在 Conversation/);
    assert.equal(client.event("tool_call"), undefined);
});

test("artifact tools always infer project scope from the current canvas", async (t) => {
    const workspace = await fs.mkdtemp(path.join(os.tmpdir(), "canvas-artifacts-"));
    const session = new CanvasSession("conversation-1", new ResearchArtifactStore(workspace));
    const client = connect(session, "first");
    t.after(async () => {
        client.close();
        await fs.rm(workspace, { recursive: true, force: true });
    });
    session.updateState(snapshot("project-current"), "first");
    session.setCodexState({ busy: true, threadId: "conversation-1", turnId: "turn-1" });

    const created = await session.callTool("research_artifact_write", {
    ...
    assert.equal(field(created, "projectId"), "project-current");
    assert.equal(field(client.event("artifacts_changed"), "artifactId"), field(created, "id"));
    assert.deepEqual(await session.callTool("research_artifact_list", {}), [created]);
    const read = await session.callTool("research_artifact_read", { artifactId: field(created, "id") });
    assert.equal(field(read, "content"), "# Seed");
    assert.equal(client.event("tool_call"), undefined);
});

test("project data reads artifacts through the requesting canvas connection", async (t) => {
    const workspace = await fs.mkdtemp(path.join(os.tmpdir(), "canvas-project-data-"));
    const store = new ResearchArtifactStore(workspace);
    const session = new CanvasSession("conversation-1", store);
    const first = connect(session, "first");
    const second = connect(session, "second");
    t.after(async () => {
        first.close();
    ...
    session.updateState(snapshot("project-a"), "first");
    session.updateState(snapshot("project-b"), "second");
    const artifactA = await store.write(
        { userId: "alice", projectId: "project-a", conversationId: "conversation-1", turnId: "turn-a" },
        { kind: "problem-evidence", title: "A 的证据", content: "# A", sourceNodeIds: ["problem-a"] },
    );
    const artifactB = await store.write(
        { userId: "alice", projectId: "project-b", conversationId: "conversation-2", turnId: "turn-b" },
        { kind: "problem-evidence", title: "B 的证据", content: "# B", sourceNodeIds: ["problem-b"] },
    );

    assert.deepEqual(await session.listArtifactsForClient("first"), [artifactA]);
    assert.equal((await session.readArtifactForClient("first", artifactA.id)).content, "# A");
    await assert.rejects(session.readArtifactForClient("first", artifactB.id), /找不到 Artifact/);
    await assert.rejects(session.listArtifactsForClient("missing"), /当前网页未连接/);
});

test("research workflow commit sends only validated canvas operations", async (t) => {
    const session = new CanvasSession();
    const client = connect(session, "first");
    t.after(() => client.close());
    session.updateState(
    ...
    const result = session.callTool("research_workflow_advance", {
        kind: "commit",
        sourceNodeIds: ["seed-1"],
        confirmation: "保留这个方向",
        candidates: [
    ...
    assert.equal(field(field(await result, "canvasResult"), "ok"), true);
});
...
test("画布写操作只发送给当前激活网页", async (t) => {
    ...
    const result = session.callTool("canvas_create_text_node", {
        text: "只写入第二个画布",
    });
    const call = second.event("tool_call");
    assert.equal(first.event("tool_call"), undefined);
    assert.equal(field(call, "name"), "canvas_apply_ops");
    session.resolveResult("second", {
        requestId: String(field(call, "requestId")),
        result: { ok: true },
    });
...
test("当前 turn 的图片附件可在发起标签页画布创建图片节点", async (t) => {
    ...
    session.setTurnAttachments("first", [
        {
            id: "attachment-1",
            name: "商品.png",
            type: "image/png",
    ...
    const result = session.callTool("canvas_create_attachment_nodes", {
    ...
    const created = (await result) as {
        nodes: Array<{ id: string; attachmentId: string; title: string }>;
    };
...
test("图片附件只允许发起 turn 的标签页读取和落入画布", async (t) => {
    ...
    await assert.rejects(
        session.callTool("canvas_create_attachment_nodes", {
            attachmentIds: ["attachment-1"],
        }),
        /发起标签页/,
    );
...
test("生成状态查询由当前激活网页返回", async (t) => {
    ...
    session.resolveResult("second", {
        requestId: String(field(call, "requestId")),
        result: { total: 1, tasks: [{ id: "image-1", status: "running" }] },
    });
...
test("shared thread events are broadcast with the active thread id", (t) => {
    ...
    session.emitThread("workspace_changed", "thread-2", {
        activeThreadId: "thread-2",
    });

    assert.deepEqual(first.event("workspace_changed"), {
        activeThreadId: "thread-2",
        threadId: "thread-2",
    });
    assert.deepEqual(second.event("workspace_changed"), {
        activeThreadId: "thread-2",
        threadId: "thread-2",
    });
});
...
test("new clients receive the current Codex state and later updates", (t) => {
    ...
    session.trackCodexEvent("codex_approval", {
    ...
    assert.deepEqual(field(hello, "conversation"), {
        revision: 1,
    ...
    assert.deepEqual(field(hello, "codex"), {
        busy: true,
        threadId: "thread-2",
        turnId: "turn-1",
    });
    ...
    session.trackCodexEvent("codex_approval_resolved", {
        requestId: "approval-1",
    });
    ...
    assert.deepEqual(session.codexPendingApprovals, []);
    assert.deepEqual(client.event("codex_state"), {
        busy: false,
        threadId: "thread-2",
        turnId: "turn-1",
    });
...
test("没有 MCP 清单时会话创建后即可 ready", () => {
    const session = new CanvasSession();
    session.beginConversation({ sourceClientId: "first" });
    const ready = session.completeConversationPreparation("thread-pi");
    assert.equal(ready.status, "ready");
    assert.equal(ready.threadId, "thread-pi");
    assert.deepEqual(ready.mcpStatuses, {});
});
...
test("可选 MCP 失败进入 warning，画布 MCP 失败进入 failed", () => {
    ...
    assert.match(failed.error || "", /CoResearch MCP/);
    ...
    assert.match(missing.error || "", /CoResearch MCP/);
});
...
test("Skill draft generation broadcasts shared busy state and restores the previous thread", (t) => {
    ...
    assert.deepEqual(first.events("codex_state").at(-1), {
        busy: true,
        threadId: "thread-1",
        turnId: "",
    });
    assert.deepEqual(second.events("codex_state").at(-1), {
        busy: true,
        threadId: "thread-1",
        turnId: "",
    });
...
test("Skill draft busy state preserves the previous turn replay until history acknowledges it", (t) => {
    const session = new CanvasSession();
    session.setCodexState({ busy: true, threadId: "thread-1", turnId: "turn-1" });
    session.emitThread("agent_event", "thread-1", {
        turnId: "turn-1",
        type: "item.updated",
        item: { id: "assistant-1", type: "agent_message", text: "回答" },
    });
    session.setCodexState({ busy: false });
    const previous = session.codexStateSnapshot;
...
test("新连接会回放当前运行 turn 的最新事件快照", (t) => {
    const session = new CanvasSession();
    session.setCodexState({ busy: true, threadId: "thread-1", turnId: "turn-1" });
    session.emitThread("chat_message", "thread-1", {
        turnId: "turn-1",
        message: {
            id: "thread-1:turn-1:synthetic:user",
    ...
    session.emitThread("agent_event", "thread-1", {
        turnId: "turn-1",
        type: "item.updated",
        item: { id: "reasoning-1", type: "reasoning", text: "分析中" },
    });
...
test("同一 item 的多次增量只回放最新内容", (t) => {
    ...
    session.emitThread("agent_event", "thread-1", {
        turnId: "turn-1",
        type: "item.updated",
        item: { id: "assistant-1", type: "agent_message", text: "第一段" },
    });
    session.emitThread("agent_event", "thread-1", {
        turnId: "turn-1",
        type: "item.updated",
        item: { id: "assistant-1", type: "agent_message", text: "第一段和第二段" },
    });
...
test("增量事件重放时转换为完整文本快照", (t) => {
    ...
    session.emitThread("agent_event", "thread-1", {
        turnId: "turn-1",
        type: "item.updated",
        item: { id: "assistant-1", type: "agent_message", delta: "第一段" },
    });
    session.emitThread("agent_event", "thread-1", {
        turnId: "turn-1",
        type: "item.updated",
    ...
    assert.deepEqual(field(event, "item"), {
        id: "assistant-1",
        type: "agent_message",
        text: "第一段第二段",
    });
...
test("并行 item 更新后重放仍保留开始顺序和命令字段", (t) => {
    ...
    session.emitThread("agent_event", "thread-1", {
        ...
        item: {
            ...
            command: "first",
            cwd: "D:\\infinite-canvas",
        },
    });
    session.emitThread("agent_event", "thread-1", {
        turnId: "turn-1",
        type: "item.started",
        item: { id: "second", type: "command_execution", command: "second" },
    });
    session.emitThread("agent_event", "thread-1", {
        turnId: "turn-1",
        type: "item.updated",
        item: { id: "first", type: "command_execution", delta: "output" },
    });
    ...
    assert.deepEqual(
        events.map((event) => field(field(event, "item"), "id")),
        ["first", "second"],
    );
    assert.deepEqual(field(events[0], "item"), {
        id: "first",
        type: "command_execution",
        command: "first",
        cwd: "D:\\infinite-canvas",
...
test("长 turn 不会淘汰仍在更新的活动条目快照", (t) => {
    ...
    t.after(() => client.close());
    assert.equal(
        (client.events("agent_event") as Array<Record<string, unknown>>).some((event) => field(field(event, "item"), "id") === "active-item"),
        true,
    );
});
...
test("开始下一 turn 时只回放当前 turn 的事件", (t) => {
    ...
    session.emitThread("agent_event", "thread-1", {
        turnId: "turn-1",
        type: "item.updated",
        item: { id: "assistant", type: "agent_message", text: "第一轮" },
    });
    ...
    session.emitThread("agent_event", "thread-1", {
        turnId: "turn-2",
        type: "item.updated",
        item: { id: "assistant", type: "agent_message", text: "第二轮" },
    });
    ...
    assert.equal(events.length, 1);
    assert.deepEqual(
        events.map((event) => field(field(event, "item"), "text")),
        ["第二轮"],
    );
});
