...
import { researchArtifactKinds } from "../artifacts/store.js";
...
const nodeTypeSchema = z.enum(["image", "text", "config", "video", "audio", "group", "web", "pdf", "seed", "direction", "research_question", "problem", "hypothesis", "approach", "method", "evaluation", "idea"]);
const generationModeSchema = z.enum(["text", "image", "video", "audio"]);
const researchWorkflowCandidateSchema = z.object({
    nodeType: z.enum(["seed", "direction", "research_question", "problem", "hypothesis", "approach", "method", "evaluation", "idea"]),
...
export const toolNames = [
    ...
    "research_workflow_advance",
    "research_artifact_write",
...
export const toolInputSchemas = {
    ...
    research_workflow_advance: z.object({
        kind: z.enum(["explore", "commit", "synthesize"]),
        sourceNodeIds: z.array(z.string()).min(1),
        action: z.string().optional(),
        confirmation: z.string().optional(),
        candidates: z.array(researchWorkflowCandidateSchema).optional(),
    }),
    research_artifact_write: z.object({
        kind: z.enum(researchArtifactKinds),
        title: z.string().min(1),
        content: z.string().min(1),
        sourceNodeIds: z.array(z.string()),
    }),
    research_artifact_list: z.object({}),
    research_artifact_read: z.object({ artifactId: z.string().min(1) }),
...
export const toolDescriptions: Record<ToolName, string> = {
    site_navigate: "跳转网站页面。path 可为 / (首页)、/canvas (我的画布)、/canvas/:id (指定画布)、/config (配置)。操作画布前若不在画布页，先用本工具打开画布。",
    ...
    research_workflow_advance:
        "推进 Seed → Direction → Research Question → Problem → Hypothesis → Approach → Method/Evaluation → Idea 研究工作流。kind=explore 只返回当前节点专属候选提示词，绝不写画布；用户明确保留后用 kind=commit，由工具校验合法下一类型并创建节点/连线；形成 Idea 时用 kind=synthesize，必须同时引用 Problem、Hypothesis、Approach、Method、Evaluation。candidates.document 是完整 Markdown（含该类型权威 ##），summary 是独立 Preview 一句核心判断。不要绕过本工具自行拼研究节点写操作。",
    research_artifact_write:
        "把用户明确要求保存或导出的研究产物不可变地写入当前 Canvas 所属 Project。仅接受 kind、title、Markdown content、sourceNodeIds；不得传入或推断路径、用户、Project。Artifact 是节点承诺的派生产物，不替代画布节点。",
    research_artifact_list: "列出当前 Canvas 所属 Project 的研究 Artifact 元数据，不返回其他 Project 的内容。",
    research_artifact_read: "按 artifactId 读取当前 Canvas 所属 Project 的一个研究 Artifact；无法读取其他 Project 的产物。",
    canvas_apply_ops:
        "批量操作当前网页画布。ops 支持 add_node、update_node、delete_node、delete_connections、connect_nodes、set_viewport、select_nodes、run_generation。研究主流程的新增和连线必须使用 research_workflow_advance；本工具可处理布局、修订、文献及普通节点。文献节点用 web/pdf，链接写在 metadata.sourceUrl。",
    canvas_create_node:
        "创建普通节点、文献节点或用户明确要求的独立起始 Seed。Seed 后的研究主流程节点必须使用 research_workflow_advance。文献用 web（网页/预印本）或 pdf，metadata.sourceUrl 放链接，metadata.summary 放一句贡献；文献库用 group。",
    ...
    canvas_update_node:
        "更新节点基础字段或 metadata。修订研究卡片时先读完整 metadata.document，再整篇提交；保留用户正文和自定义 ##，权威 ## 缺失时只追加。核心含义变化时同步 metadata.summary；summary 是卡片 Preview，不得从 Markdown 自动摘取。",
    canvas_update_node_text: "更新节点正文和标题。研究卡片写入 metadata.summary；文本节点写入 metadata.content。",
