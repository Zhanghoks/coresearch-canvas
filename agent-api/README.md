...
不支持 Project 成员、角色或共享 Workspace。封闭测试通过一次性邀请码注册（`POST /v1/auth/register`），邀请码表仅 service role 可写。运行时编辑真值仍在浏览器；服务端保存 Canvas snapshot 用于恢复。`agent-api` 是浏览器唯一 Gateway，浏览器不直连 Codex。

默认运行时仍是 Pi。切换托管 Codex 时设置 `AGENT_RUNTIME=codex`，并执行 `supabase/migrations/002_codex_runtime.sql`。

## 配置

1. 在 Supabase SQL Editor 依次执行 `supabase/migrations/001_agent_platform.sql`、`002_codex_runtime.sql`（若用 Codex）和 `003_invite_codes.sql`。
...
`SUPABASE_SECRET_KEY` 只用于服务端初始化、建号和写邀请码，不会写入浏览器运行时配置。产品登录后使用平台 `PI_API_KEY` / `CODEX_API_KEY`，用户不填写模型 Key。
