# CoResearch Problem Formation / Problem Framing 设计层

- 日期：2026-09-17
- 状态：设计提案；本次不实现页面、Skill、服务或真实文献检索。
- 上游：Research Question Formation、Evaluation Exploration 与 Research Design Co-Design。
- 关联：[Step 2 · Research Landscape](./02-research-landscape.md)、[Idea 结构设计](../idea-structure.md)。
- 下游：Research Design Candidate / Idea Draft。
- 产品流程真值：[Research Flow](../research-flow.md)。Problem 是 Research Design 内部的构造层，不是 RQ 之后强制先行的独立步骤；必须保留证据和不确定性，不能写 “No one has done this”。

## 当前对齐契约

本文保留 Problem 的 claim 拆解、Targeted Search、Scope、Evidence Status 和确认规则，但不再作为 `Research Question → Problem → Hypothesis → Method` 的线性导航。当前顺序是：先从 Paper Sections 形成 RQ，再探索 Evaluation Strategy 和 Method Candidates；Problem、Mechanism、Predictions、Falsifiers 和 Approach 在 Research Design Co-Design 中按需要补充。Problem 不能绕过已确认的 Evaluation Targets 直接触发 Method。
