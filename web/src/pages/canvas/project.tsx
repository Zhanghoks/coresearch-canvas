...
import { cleanupUnusedImages, ensureImagePreview, uploadImage } from "@/services/image-storage";
import { cleanupUnusedMedia, uploadMediaFile } from "@/services/file-storage";
...
import { CanvasNodeDocumentModal } from "@/components/canvas/canvas-node-document-modal";
...
import { closeNodeDocumentSession, openNodeDocumentSession } from "@/lib/canvas/research-node-agent";
import { findWorkspaceCanvasProject, useCanvasStore, workspaceCanvasProjects } from "@/stores/canvas/use-canvas-store";
...
import { audioMetadata, createCanvasNode, imageMetadata, videoMetadata } from "@/lib/canvas/canvas-node-factory";
import { RESEARCH_SOURCE_MIME, researchSourceNodeType, type ResearchSourceCard } from "@/lib/canvas/research-source-cards";
...
import { hydrateCanvasImages, resetInterruptedGeneration } from "@/lib/canvas/canvas-media-hydrate";
import { audioExtension, imageExtension, isAudioFile } from "@/lib/canvas/canvas-media-format";
...
function InfiniteCanvasPage() {
    ...
    const currentProject = useMemo(() => findWorkspaceCanvasProject(projects, projectId), [projects, projectId, signedInUserId]);
    ...
    const cleanupCanvasFiles = useCallback((extra?: unknown) => {
        window.setTimeout(async () => {
            const projects = useCanvasStore.getState().projects;
            await cleanupUnusedImages({ assets: [], projects, extra: { ...((extra as object) || {}), history: historyRef.current, lastHistory: lastHistoryRef.current } });
            await cleanupUnusedMedia({ assets: [], projects, extra: { ...((extra as object) || {}), history: historyRef.current, lastHistory: lastHistoryRef.current } });
        }, 0);
    }, []);
    ...
    useEffect(() => {
        ...
        let cancelled = false;
        const restore = async () => {
            const project = (await loadProject(projectId)) || openProject(projectId);
            if (cancelled) return;
            if (!project) {
    ...
    }, [hydrated, loadProject, navigate, openProject, projectId]);
    ...
    useEffect(() => {
        if (!projectLoaded || restoredProjectIdRef.current !== projectId || historyPausedRef.current) return;
    ...
    const canvasToScreen = useCallback((world: Position) => {
        const rect = containerRef.current?.getBoundingClientRect();
        const currentViewport = viewportRef.current;
        return {
            x: (rect?.left || 0) + currentViewport.x + world.x * currentViewport.k,
            y: (rect?.top || 0) + currentViewport.y + world.y * currentViewport.k,
        };
    }, []);
    ...
    const createConnectedNode = useCallback(
        (type: CanvasNodeTypeId, pending: PendingConnectionCreate) => {
            ...
            if (definition?.Panel && definition.autoOpenPanel && !definition.hidePanel) setDialogNodeId(newNode.id);
    ...
    const documentNode = documentNodeId ? nodeById.get(documentNodeId) || null : null;
    ...
    const createNode = useCallback(
        (type: CanvasNodeTypeId, position?: Position) => {
            const newNode = createCanvasNode(type, position || getCanvasCenter());
            setNodes((prev) => [...prev, newNode]);
            setSelectedNodeIds(new Set([newNode.id]));
            setSelectedConnectionId(null);
            const definition = getNodeDefinition(type);
    ...
    const deleteNodes = useCallback(
        (ids: Set<string>) => {
            ...
            setDocumentNodeId((current) => (current && allIds.has(current) ? null : current));
    ...
    const createAndOpenProject = useCallback(() => {
        const projectCount = workspaceCanvasProjects(useCanvasStore.getState().projects).length;
        const id = createProject(t("canvas.defaultTitle", { count: projectCount + 1 }));
        navigate(`/canvas/${id}`);
    }, [createProject, navigate, t]);

    const deleteCurrentProject = useCallback(() => {
        useCanvasUiStore.getState().setDeleteProjectIds([projectId]);
    }, [projectId]);

    const exportCurrentProject = useCallback(async () => {
        const project = findWorkspaceCanvasProject(useCanvasStore.getState().projects, projectId);
    ...
    const handleDrop = useCallback(
        (event: ReactDragEvent<HTMLDivElement>) => {
            event.preventDefault();
            const directionRaw = event.dataTransfer.getData(RESEARCH_DIRECTION_MIME);
            if (directionRaw) {
                try {
                    const card = JSON.parse(directionRaw) as ResearchDirectionCard;
                    if (card?.title) {
                        const position = screenToCanvas(event.clientX, event.clientY);
                        const node = createCanvasNode(CanvasNodeType.Direction, position, { summary: card.summary || "", document: card.document || "", status: "idle" });
            ...
            const sourceRaw = event.dataTransfer.getData(RESEARCH_SOURCE_MIME);
            if (sourceRaw) {
                try {
    ...
    const handleOpenDocument = useCallback((node: CanvasNodeData) => {
        setDocumentNodeId(node.id);
        openNodeDocumentSession(node);
    }, []);
    const handleCloseDocument = useCallback(() => {
        setDocumentNodeId(null);
        closeNodeDocumentSession();
    }, []);
    useEffect(() => {
        if (documentNodeId) return;
        closeNodeDocumentSession();
    }, [documentNodeId]);
    ...
    const renderNodePanel = useCallback(
        (panelNode: CanvasNodeData) => (getNodeDefinition(panelNode.type)?.Panel ? renderPluginPanel(panelNode) : null),
        [renderPluginPanel],
    );
    ...
    return (
        <main className="flex h-full min-h-0 overflow-hidden" style={{ background: theme.canvas.background, color: theme.node.text }}>
            <section className="relative min-w-0 flex-1 overflow-hidden">
                ...
                >
                    ...
                    {visibleNodes.map((node) => (
                        <CanvasNode
                            ...
                            showPanel={!isNodeResizing && !selectionBox && selectedNodeIds.size === 1 && selectedNodeIds.has(node.id) && !getNodeDefinition(node.type)?.hidePanel}
                ...
                {!nodes.length && !nodeCreatePosition ? <CanvasEmptyState onAddNode={createNode} onOpenAgent={openAgentPanel} /> : null}
                {nodeCreatePosition ? (
                    <NodeCreateMenu
                        position={canvasToScreen(nodeCreatePosition)}
                        onCreate={(type) => {
                ...
                <CanvasNodeHoverToolbar
                    node={documentNodeId || isNodeDragging || isNodeResizing || expandedBatchNodeIds.has(toolbarNode?.id || "") ? null : toolbarNode}
                    viewport={viewport}
                ...
                <CanvasNodeDocumentModal
                    node={documentNode}
                    open={Boolean(documentNode)}
                    onClose={handleCloseDocument}
                    onSave={(nodeId, document) => setNodes((current) => current.map((node) => (node.id === nodeId ? { ...node, metadata: { ...node.metadata, document } } : node)))}
                />
