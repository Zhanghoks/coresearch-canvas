# CoResearch Hypothesis / Mechanism 设计层

- 日期：2026-09-17
- 状态：设计提案；本次不实现页面、Skill、服务或真实文献检索。
- 上游：Research Question、Evaluation Targets 与 Research Design Candidate。
...
- 下游：Research Design Co-Design。

## 当前对齐契约

Hypothesis 不再是 RQ 后独立阻塞 Method 的固定步骤，而是 Research Design 中用于解释机制、生成 Predictions 和 Falsifiers 的设计层。它仍必须可证伪、区别于 Problem，也必须与已确认的 Evaluation Targets 对齐；没有可观察的 Evaluation，不得把 Hypothesis 写成已成立机制。
- 产品流程真值：[Research Flow](../research-flow.md)。本文内容作为 Research Design Co-Design 内的机制解释层复用；不再作为 RQ 后独立的顺序步骤。

## 1. Step 5 解决什么

**在确认的 Research Problem 与任何方法设计之间，插入一层显式的机制解释：用户必须先说明“我认为这个问题为什么会发生”，并把它写成可证伪的假设。**

当前设计层的输入是 Research Question、Evaluation Targets，以及需要解释的 Problem / Limitation：

```text
Research Question   我准备继续追问什么
Evaluation Target  什么观测能够支持或反驳解释
Problem / Limitation  哪个困难需要机制解释
```
