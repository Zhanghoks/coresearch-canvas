...
export function startHttpServer() {
    ...
    applySearchEnv();
    void applyCredentialsFromEnv().catch((error) => {
        logger.warn("Failed to apply model credentials from environment", { error: error instanceof Error ? error.message : String(error) });
    });
    ...
    app.get("/projects", route(async (req, res) => {
        res.json({ ok: true, data: await projectStore.list(requestUserId(req)) });
    }));
    app.get("/projects/:projectId/location", route(async (req, res) => {
        const userId = requestUserId(req);
        const projectId = routeParam(req.params.projectId);
        await projectStore.get(userId, projectId);
        res.json({ ok: true, path: projectStore.dir(userId, projectId) });
    }));
    app.post("/projects/:projectId/reveal", route(async (req, res) => {
        const userId = requestUserId(req);
        const projectId = routeParam(req.params.projectId);
        await projectStore.get(userId, projectId);
        const directory = projectStore.dir(userId, projectId);
        await revealLocalFile(directory, true);
        res.json({ ok: true, path: directory });
    }));
    app.get("/projects/:projectId", route(async (req, res) => {
        res.json({ ok: true, data: await projectStore.get(requestUserId(req), routeParam(req.params.projectId)) });
    }));
    app.put("/projects/:projectId", route(async (req, res) => {
        const userId = requestUserId(req);
        const projectId = routeParam(req.params.projectId);
        const record = await projectStore.put(userId, { ...(req.body || {}), id: projectId, ownerUserId: userId });
        await mkdir(path.join(projectStore.dir(userId, projectId), ".agents", "skills"), { recursive: true }).catch(() => undefined);
        res.json({ ok: true, data: record, path: projectDir(defaultDataRoot(), userId, projectId) });
    }));
    app.delete("/projects/:projectId", route(async (req, res) => {
        await projectStore.delete(requestUserId(req), routeParam(req.params.projectId));
        res.json({ ok: true });
    }));
    ...
    app.get("/data/artifacts", route(async (req, res) => {
        const data = await session.listArtifactsForClient(String(req.query.clientId || ""));
        res.setHeader("Cache-Control", "no-store");
        res.json({ ok: true, data });
    }));
    app.get("/data/artifacts/:artifactId", route(async (req, res) => {
        const data = await session.readArtifactForClient(String(req.query.clientId || ""), routeParam(req.params.artifactId));
        res.setHeader("Cache-Control", "no-store");
        res.json({ ok: true, data });
    }));
    ...
    app.put("/agent/credentials", route(async (req, res) => {
        ...
        res.json({ ok: true, runtime: "pi", data: await listProviderStatus() });
    }));
    app.delete("/agent/credentials/:providerId", route(async (req, res) => {
        await deleteProviderApiKey(routeParam(req.params.providerId));
        res.json({ ok: true, runtime: "pi", data: await listProviderStatus() });
    }));
    app.get("/agent/search-credentials", route(async (_req, res) => res.json({ ok: true, runtime: "pi", data: listSearchApiStatus() })));
    app.put("/agent/search-credentials", route(async (req, res) => {
        setSearchApiValue(String(req.body?.apiId || ""), String(req.body?.value || ""));
        res.json({ ok: true, runtime: "pi", data: listSearchApiStatus() });
    }));
    app.delete("/agent/search-credentials/:apiId", route(async (req, res) => {
        deleteSearchApiValue(routeParam(req.params.apiId));
        res.json({ ok: true, runtime: "pi", data: listSearchApiStatus() });
    }));
    const sendModels = async (_req: Request, res: Response) => res.json({ ok: true, ...(await listCodexModels(emit)) });
    ...
    app.post("/agent/codex/skills/draft", codexMutation(async (req, res) => {
        ...
        const effort = reasoningEffort(req.body?.thinkingLevel || req.body?.effort);
    ...
    app.post("/agent/codex/threads/:threadId/fork", codexMutation(async (req, res) => {
        const sourceThreadId = routeParam(req.params.threadId);
        const clientId = String(req.body?.clientId || "");
        ...
        session.beginConversation({ sourceClientId: clientId });
        emit("agent_bootstrap", { type: "codex.preparing", threadId: sourceThreadId, sourceClientId: clientId || undefined });
        try {
            const result = await forkCodexThread(emit, sourceThreadId, workspace.workspacePath, permission, lastTurnId, true);
            const nextThreadId = String((result.thread as { id?: string }).id || "");
            if (!nextThreadId) throw new Error("没有返回 thread id");
            session.completeConversationPreparation(nextThreadId);
            const nextWorkspace = setActiveThread(nextThreadId, { sourceClientId: clientId, forkedFromId: sourceThreadId }, true);
            res.json({ ok: true, workspace: nextWorkspace, conversation: session.conversationStateSnapshot, ...result, thread: summarizeCodexThread(result.thread) });
        } catch (error) {
            failPreparedConversation(error, sourceThreadId, clientId);
            throw error;
        }
    }));
    ...
    app.post("/agent/codex/turn", codexMutation(async (req, res) => {
        ...
        if (!activeThreadId || !["ready", "warning"].includes(conversation.status)) {
            return res.status(409).json({ ok: false, code: "CONVERSATION_NOT_READY", error: "Agent 对话仍在初始化，请稍后再试", state: conversation });
        }
        const model = String(req.body?.model || "") || undefined;
        const effort = reasoningEffort(req.body?.thinkingLevel || req.body?.effort);
    ...
    app.post("/agent/codex/permission-mode", route(async (req, res) => {
        const mode = syncAgentPermissionMode(permissionMode(req.body?.permissionMode));
        res.json({ ok: true, permissionMode: mode });
    }));
    ...
    app.listen(port, "127.0.0.1", () => {
        console.log("CoResearch Agent");
        checkVersions();
        console.log(`Local URL: ${config.url}`);
        console.log(`Connect token: ${config.token}`);
        console.log("Sidebar Agent uses an embedded Pi runtime. Codex MCP is optional for the Codex App plugin.");
...
function permissionMode(value: unknown): AgentPermissionMode {
    return resolveAgentPermissionMode(value);
}

function reasoningEffort(value: unknown): CodexReasoningEffort | undefined {
    return value === "off" || value === "minimal" || value === "low" || value === "medium" || value === "high" || value === "xhigh" || value === "max" || value === "ultra" ? value : undefined;
}
