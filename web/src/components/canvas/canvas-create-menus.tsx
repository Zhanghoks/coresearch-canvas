...
export function ConnectionCreateOption({ theme, icon, title, description, onClick }: { theme: (typeof canvasThemes)[keyof typeof canvasThemes]; icon: React.ReactNode; title: string; description?: string; onClick?: () => void }) {
    return (
        <button
            type="button"
            className={`flex w-full cursor-pointer items-center gap-3 rounded-2xl px-3 text-left transition ${description ? "h-16" : "h-11"}`}
            style={{ color: theme.node.text }}
            onClick={onClick}
            onMouseEnter={(event) => (event.currentTarget.style.background = theme.node.fill)}
            onMouseLeave={(event) => (event.currentTarget.style.background = "transparent")}
        >
            <span className={`grid shrink-0 place-items-center rounded-xl ${description ? "size-11" : "size-9"}`} style={{ background: theme.node.fill, color: theme.node.muted }}>
                {icon}
            </span>
...
export function NodeCreateMenu({ position, onCreate, onClose }: { position: Position; onCreate: (type: string) => void; onClose: () => void }) {
    ...
    return (
        <div
            ref={menuRef}
            className="fixed z-[120] max-h-[70vh] w-[300px] overflow-y-auto rounded-[18px] border p-3 shadow-2xl thin-scrollbar"
            data-canvas-no-zoom
