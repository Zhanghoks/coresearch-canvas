...
- 上游：Evaluation Exploration、Method Exploration，以及按需补充的 Problem / Hypothesis / Approach。
- 画布：[Huabu 领域映射与交互升级提案](../canvas/huabu-domain-binding.md)。
- 下游：Idea Draft / Idea Review。
- 产品流程真值：[Research Flow](../research-flow.md)。本步对应 Method & Evaluation Co-Design：Claim ↔ Method ↔ Evaluation 同时设计，仍不做实验协议。

## 当前对齐契约

本步是 Research Design Co-Design，不是旧流程中先确认 Method 再补 Evaluation。输入必须已有 Evaluation Targets / Strategy 与 Method Candidates；输出是可比较、可追溯的 Research Design Candidate：Research Question、Core Claim、Method、Evaluation、Supporting Literature、Known Risks。Problem、Mechanism、Predictions、Falsifiers 和 Approach 作为设计层补充，具体 benchmark、样本数、threshold、run order 和算力仍属于后续 Experiment Planning。

## 1. Step 7 解决什么

到 Evaluation Exploration 和 Method Exploration 完成后，用户已经有可比较的 Evaluation Strategy 与 Method Candidates，例如：
...
本步不把用户带到一张新的实验表单，而是在同一个 Huabu Space 中回答三件事：
