...
export const CanvasNode = React.memo(function CanvasNode({
    ...
    useEffect(() => {
        if (!isSelected) setTagsOpen(false);
    }, [isSelected]);
    ...
    const handleResizeMove = useCallback(
        (event: MouseEvent) => {
            ...
            const minHeight = isResearchFlow ? 360 : 160;
    ...
    return (
        ...
        >
            {!referenceSelectionState && (isResearchFlow || isSelected || hovered || isEditingTitle) && (
                <div className="absolute left-2 top-[-26px] z-[65] flex max-w-[calc(100%-44px)] items-center gap-2" onMouseDown={(event) => event.stopPropagation()} onPointerDown={(event) => event.stopPropagation()}>
                    {isEditingTitle ? (
                        <input
                            ref={titleInputRef}
                            value={titleDraft}
                        ...
                        <button
                            type="button"
                            className="flex min-w-0 items-center gap-1.5 truncate px-0 py-0.5 text-left text-xs font-medium opacity-90 transition hover:opacity-100"
                        ...
                        <button
                            type="button"
                            className="block min-w-0 truncate border-b border-dashed border-transparent px-0 py-0.5 text-left text-xs font-medium opacity-75 transition hover:border-current hover:opacity-100"
                            style={{ color: theme.node.text }}
                            title={t("canvas.node.renameHint")}
                            onDoubleClick={(event) => {
                                event.stopPropagation();
                    ...
                    {isResearchFlow ? <CanvasNodeTagChips node={data} theme={theme} /> : null}
                </div>
            )}

            {isResearchFlow && !referenceSelectionState && (isSelected || hovered || tagsOpen) ? (
                <div className="absolute right-3 top-[-14px] z-[66]">
                    <CanvasNodeTagsButton open={tagsOpen} theme={theme} onToggle={() => setTagsOpen((value) => !value)} />
                </div>
            ) : null}

            {isResearchFlow && tagsOpen && pluginContext ? (
                <div className="absolute left-full top-0 z-[80] ml-3">
                    <CanvasNodeTagsPanel node={data} theme={theme} onChange={(patch) => pluginContext.updateMetadata(patch)} />
                </div>
            ) : null}
            ...
            <div
                ...
                onDoubleClick={(event) => {
                    ...
                    if ((RESEARCH_FLOW_NODE_TYPES as readonly string[]).includes(data.type)) {
                        event.stopPropagation();
                        onDocument?.(data);
                        return;
                    }
            ...
            {panelContent ? (
                <div
                    className="absolute left-1/2 top-full z-[70] flex w-max justify-center pt-4"
                    style={{
                        transform: isResearchFlow ? `translateX(-50%) scale(${1 / Math.max(scale, 0.05)})` : "translateX(-50%)",
