...
本文与当前 Flow 对齐：Seed 是用户兴趣的轻量起点，状态可经历 `initial → refining → ready_for_exploration`，但不提前创建 Direction、Research Question、Evaluation 或 Method。Seed 确认与“开始 Direction Exploration”是两个动作；SearchBrief 只是派生检索工件。后续所有 Direction、Paper Section 和 Idea Section 都通过 `seedRef` 追溯到具体 Seed revision。
