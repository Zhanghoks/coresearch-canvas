...
import { isResearchFlowNodeType, type CanvasNode, type CanvasNodeType, type CanvasSnapshot } from "./types.js";
...
export function buildCanvasToolRequest(name: ToolName, input: Record<string, unknown>, state: CanvasSnapshot | null): CanvasToolRequest {
    ...
    if (name === "canvas_update_node_text") {
        ...
        const research = current ? isResearchFlowNodeType(current.type) : false;
        return applyOps([{ type: "update_node", id: data.id, patch: { ...(data.title ? { title: data.title } : {}) }, metadata: research ? { summary: data.text, content: data.text, status: "success" } : { content: data.text, status: "success" } }]);
    }
