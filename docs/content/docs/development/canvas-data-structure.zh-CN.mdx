...
- 视频节点：`content` 是当前可播放的视频 URL，通常是 `blob:` URL；`storageKey` 指向本地视频 Blob；`bytes/mimeType` 保存文件信息。旧项目数据里可能仍带有 `videoTaskId` 等生成任务字段，当前前端不再轮询恢复。
- 文本节点：`content` 保存文本内容；`fontSize` 保存字体大小；`status/errorDetails` 等字段可能来自历史数据。
- 配置节点：仍可能带有历史生成相关 metadata；当前产品不再提供前端生成入口。
- 多图图片节点：一个节点可含 `images` 数组与 `primaryImageId`；展开状态仅属于当前界面，不写入项目数据。
