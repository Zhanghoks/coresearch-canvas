...
本地 Canvas Agent 连接画布网页和本机 Agent 运行时。侧边栏对话内嵌 Pi SDK，不需要安装 Codex CLI。Codex App 插件仍可通过 `canvas-agent mcp` 操作画布。本地开发时优先连接 `http://localhost:3000`，不需要先使用线上站点。
...
需要排查连接、线程或工具调用问题时，可开启 Debug 模式：

```bash
npx -y @basketikun/canvas-agent@latest --debug
```

Debug 日志会以 `[DEBUG][HH:mm:ss]` 等传统格式输出到终端，并按启动日期保存到仓库 `.data/logs/coresearch-YYYY-MM-DD.log`。终端日志带级别颜色，文件日志为纯文本；日志包含 HTTP、SSE、线程、turn 和工具调用事件，token、API Key 与图片 Data URL 会自动隐藏。
...
在画布右侧 Agent 面板连接后，到 Setup 填写模型 API Key（写入仓库 `.data/auth.json`），即可开始对话。论文搜索可选密钥写在同一页的文献搜索 API 列表，写入 `.data/search.json`。会话落在 `.data/sessions/`，模型配置落在 `.data/runtime/`。Agent 指令和 Skill 落在 `.coresearch/`。不需要先登录 Codex。

Codex app 插件会读取启动输出里的 Local URL 和 Connect token，并直接打开画布网页地址；Canvas Agent 不负责生成画布打开 URL。

Canvas Agent 默认只监听 `127.0.0.1`。网页第一次带正确 token 连接后，Canvas Agent 会记录该网页 Origin；之后其他 Origin 不能复用这个本地 Agent，除非用户清理仓库 `.data/agent.json` 里的 `origins`。
...
仓库内提供了 Codex app 插件：`plugins/infinite-canvas`。在 Codex app 中添加本仓库的 marketplace 后，可以安装 `CoResearch` 插件；插件会注册同一个 `infinite-canvas` MCP，并带上画布操作说明。
...
使用时可以直接在 Codex 里说“打开 CoResearch”，插件会启动本地 Agent，读取 Local URL 和 Connect token，然后在右侧打开 `https://canvas.best/` 并自动新建、连接画布；只有明确要求使用本地项目时才会启动本地前端。
...
- `research_workflow_advance`（研究主流程；配合 `canvas-agent/skills/` 节点 Skill 仓库）
- `research_artifact_write`（把 Markdown 派生产物不可变地保存到当前 Canvas Project）
...
`canvas-agent/skills/` 为内置 Skill 仓库：九种研究节点分别约束提示词与工作流，`research-flow` 负责路由，`research-workspace` 负责 Artifact。Artifact Tool 不接受用户、Project 或路径参数，作用域由当前 Canvas 推导。
