...
export class CodexAppClient {
    ...
    static async start(emit: AgentEmit, onExit: () => void) {
        ...
        await client.request("initialize", { clientInfo: { name: "canvas-agent", title: "CoResearch Agent", version: VERSION }, capabilities: { experimentalApi: true, requestAttestation: false } });
    ...
    /** 从指定线程分叉出新会话；可选 lastTurnId 保留到该 turn（含）并丢弃之后的历史。 */
    async forkThread(threadId: string, cwd?: string, permissionMode: AgentPermissionMode = "request", lastTurnId?: string, preheat = false) {
        if (preheat) this.pendingPreheatThreadStarts += 1;
        let nextThreadId = "";
        try {
            const { thread } = await this.request("thread/fork", {
                threadId,
                ...threadSettings(permissionMode),
                ...(cwd ? { cwd } : {}),
            ...
            if (preheat) {
                this.preheatingThreadIds.add(nextThreadId);
                await this.completeMcpPreheat(nextThreadId);
            }
            return thread;
        } finally {
            if (preheat) this.pendingPreheatThreadStarts -= 1;
            if (nextThreadId) this.preheatingThreadIds.delete(nextThreadId);
        }
    }
    ...
    private handle(message: JsonRecord) {
        ...
        if (message.error && this.pending.has(id)) {
            ...
            if (!this.pending.get(id)?.silent) {
                if (/not materialized yet.*includeTurns|paginated threads do not support thread\/read/i.test(error)) logger.debug("Codex thread history is not fully readable", { id });
