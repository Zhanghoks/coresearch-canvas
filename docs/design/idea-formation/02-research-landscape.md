# CoResearch Step 3：Direction Deep Dive / Research Map 设计

- 日期：2026-09-17
- 状态：设计提案，尚未实现或执行真实 Deep Dive 调研。
- 上游：[Step 1 · Seed Framing](./01-seed.md)。
- 关联：[空间设计](../space.md)、[Idea 结构设计](../idea-structure.md)。
- 下游：Research Question Formation。产品流程真值见 [Research Flow](../research-flow.md)：Direction Exploration 先于本文，本文只处理用户选中的 Direction；产物是由 Paper Sections 支撑的 Direction Research Map / Dossier，而不是全领域地图。若与 Flow 冲突，以 Flow 为准。

## 当前对齐契约

本文不再定义“先建全领域 Landscape、再选择 Focus”的产品步骤。当前 Deep Dive 固定为：
...
→ 逐篇论文理解
→ Paper Section 抽取
→ Direction Research Map
```

最小证据单元是 `Paper → Paper Section`，Section 类型为 `problem`、`method`、`evaluation`、`result`、`limitation`。Direction Map 分为 Paper Evidence 和 Direction Synthesis 两层；所有综合判断必须能回到 Section 和原文位置。本文后续原有的 Work / Version / 阅读 / coverage / temporal verification 章节仍可复用，但它们服务于 Selected Direction，不再生成固定数量的全领域路线。

## 1. 目标与边界

**从已确认的 Seed 出发，建立有来源、有时间范围、可追问的研究地图：主要路线如何形成、方法如何变化、已经做到什么，以及哪些问题仍需调查。**

本步骤的完成产物是 DirectionResearchMap，不是用户的研究方向决定，也不是创新性证明。它可以归纳选定 Direction 的 Paper Sections 和路线脉络，但不能替用户确认 gap 或生成正式 Idea。
...
```ts
interface DirectionDeepDiveRequest {
  projectId: string
  seedRef: { id: string; revision: number; contentHash: string }
  directionRef: { id: string; revision: number; contentHash: string }
...
```ts
interface DirectionResearchMap {
  id: string
  revision: number
  seedRef: { id: string; revision: number; contentHash: string }
  directionRef: { id: string; revision: number; contentHash: string }
...
```text
Confirmed Seed + Selected Direction + SearchBrief
 → 聚焦检索计划 → 论文身份与版本解析 → Research Wiki 摄入
 → 逐篇阅读 → Paper Section 抽取 → Paper Evidence
 → Direction Synthesis → Direction Research Map 投影
```
