...
import { resolveAgentPermissionMode } from "./permission-mode.js";
...
/** 从指定线程分叉，可选截断到 lastTurnId，并返回新线程历史。 */
export async function forkCodexThread(emit: AgentEmit, threadId: string, cwd?: string, permissionMode: AgentPermissionMode = "request", lastTurnId?: string, preheat = false) {
    const app = await getCodexApp(emit);
    const thread = await app.forkThread(threadId, cwd, permissionMode, lastTurnId, preheat);
    const nextThreadId = String(field(thread, "id") || "");
    if (!nextThreadId) throw new Error("Codex app-server 没有返回 thread id");
    loadedThreadId = nextThreadId;
    const history = await loadCodexHistory(emit, nextThreadId, cwd);
    const supplementalItems = await codexEventHistory.readThread(nextThreadId);
    const messages = await mergeMessageMetadata(nextThreadId, threadMessages(history.thread, app.planUpdates(nextThreadId), supplementalItems));
...
async function runCodexTurnNow(prompt: string, lifecycleEmit: AgentEmit, attachments: AgentAttachment[], options: CodexRunOptions) {
    ...
    try {
        ...
        try {
            await app.startTurn(threadId, prompt, files, resolveAgentPermissionMode(options.permissionMode), options.model, options.effort, options.onTurn, options.skill, options.messageText);
        } catch (error) {
            if (!isRecoverableThreadError(error)) throw error;
...
async function ensureCodexThread(app: CodexAppClient, options: CodexRunOptions, emit: AgentEmit) {
    if (options.threadId) {
        ...
        try {
            await resumeLoadedThread(app, options.threadId, options.cwd, resolveAgentPermissionMode(options.permissionMode), true);
    ...
    if (!loadedThreadId) {
        const thread = await app.startThread(options.cwd, resolveAgentPermissionMode(options.permissionMode));
        loadedThreadId = String(field(thread, "id") || "");
    }
    return loadedThreadId;
}
