...
import { Bot, History, PanelRightClose, PlugZap, Sparkles, Terminal } from "lucide-react";
...
import {
    ...
    keepThroughTurnIdBeforeUserMessage,
    isCanvasWriteTool,
    ...
    userTextWithCanvasReferences,
    reasoningActivityText,
    registerLiveAgentTurn,
    replaceSyntheticUserMessage,
    settleStoppedActivities,
    scopeChatItem,
    stringText,
    toolName,
    turnPlanStatus,
...
const AGENT_REASONING_EFFORTS = new Set<AgentReasoningEffort>(["off", "minimal", "low", "medium", "high", "xhigh", "max", "ultra"]);
...
type AgentHelloEvent = { ok?: boolean; protocolVersion?: number; runtime?: string; clientId?: string; workspace?: { activeThreadId?: string }; conversation?: AgentConversationState; runtimeState?: AgentCodexState; codex?: AgentCodexState; pendingApprovals?: AgentPendingApproval[] };
...
function conversationBootstrapView(conversation: AgentConversationState) {
    const bootstrapStatus: AgentBootstrapStatus | null = conversation.status === "idle" || conversation.status === "preparing"
        ? { key: "agent:preparing", text: rt("conversationInitializing"), detail: rt("conversationCreating"), status: "running" }
        : conversation.status === "failed"
            ? { key: "agent:prepare_failed", text: rt("conversationInitFailed"), detail: conversation.error || rt("conversationCreateFailed"), status: "error" }
            : null;
    return { bootstrapStatus, mcpStartupStatuses: {} as Record<string, AgentBootstrapStatus> };
}
...
export function LocalAgentPanel({ embedded, headless, autoConnect }: { embedded?: boolean; headless?: boolean; autoConnect?: boolean }) {
    ...
    const { width, url, token, connected, enabled, prompt, attachments, sending, waiting, tokenUsage, eventLogs, threads, activeThreadId, workspacePath, loadingThreads, activeTab, confirmTools, permissionMode, models, model, reasoningEffort, activity, conversation, connectError, pendingTool, pendingApprovals, pendingSend, documentSession } = useAgentStore(
        useShallow((state) => ({
            width: state.width,
            url: state.url,
            token: state.token,
    ...
    const editingUserMessageRef = useRef<{ messageId: string; keepThroughTurnId: string | null } | null>(null);
    ...
    const [searchApis, setSearchApis] = useState<AgentSearchApiStatus[]>([]);
    ...
    const flushReasoning = () => {
        if (reasoningFrameRef.current) {
            cancelAnimationFrame(reasoningFrameRef.current);
            reasoningFrameRef.current = 0;
        }
        const queued = reasoningPendingRef.current;
        if (!queued.size) return;
        reasoningPendingRef.current = new Map();
        let messages = useAgentStore.getState().messages;
        queued.forEach((message) => {
            messages = upsertAgentMessage(messages, message);
        });
        setAgentState({ messages });
    };
    const queueReasoning = (id: string, patch: (activityItems: Record<string, string>, current: AgentChatItem | undefined) => AgentChatItem) => {
        const pending = reasoningPendingRef.current.get(id);
        const current = pending || useAgentStore.getState().messages.find((item) => item.id === id);
        const activityItems = { ...(current?.activityItems || {}) };
        reasoningPendingRef.current.set(id, patch(activityItems, current));
        if (reasoningFrameRef.current) return;
        reasoningFrameRef.current = requestAnimationFrame(flushReasoning);
    };
    useEffect(() => () => {
        if (reasoningFrameRef.current) cancelAnimationFrame(reasoningFrameRef.current);
    }, []);
    ...
    useEffect(() => {
        ...
        source.addEventListener("hello", (event) => {
            ...
            const codex = hello?.runtimeState || hello?.codex;
        ...
        source.addEventListener("codex_state", (event) => {
            ...
            enqueueEvent(async () => {
                ...
                const bound = activeTurnId ? bindPendingTurnMessages(current.messages, current.activeThreadId, activeTurnId) : current.messages;
                const messages = busy ? bound : settleStoppedActivities(bound, current.activeThreadId, turnId);
                setAgentState({
        ...
        source.addEventListener("chat_message", (event) => {
            ...
            enqueueEvent(() => {
                ...
                const messages = data.message!.role === "user"
                    ? replaceSyntheticUserMessage(current.messages, { ...next, clientMessageId: clientMessageId || next.clientMessageId })
                    : upsertAgentMessage(current.messages, next);
                setAgentState({ messages });
            });
        });
        ...
        source.addEventListener("artifacts_changed", () => {
            if (!isCurrentConnection()) return;
            const state = useAgentStore.getState();
            state.setAgentState({ artifactRevision: state.artifactRevision + 1 });
        });
    ...
    const loadModels = useCallback(() => {
        if (!connected) return;
        void fetchAgentModels(endpoint, token).then(({ data = [] }) => {
            ...
            const models = data.flatMap((item) => {
                ...
                const thinkingLevels = (item.thinkingLevels?.length ? item.thinkingLevels : item.supportedReasoningEfforts.map((effort) => effort.reasoningEffort))
                    .filter((level): level is AgentReasoningEffort => AGENT_REASONING_EFFORTS.has(level));
                names.add(name);
                const defaultLevel = item.defaultThinkingLevel || item.defaultReasoningEffort;
                const defaultReasoningEffort = thinkingLevels.includes(defaultLevel as AgentReasoningEffort) ? defaultLevel as AgentReasoningEffort : thinkingLevels[0] || "";
                return [{
                    ...item,
                    thinkingLevels,
                    defaultThinkingLevel: defaultReasoningEffort,
                    defaultReasoningEffort,
                    supportedReasoningEfforts: thinkingLevels.map((reasoningEffort) => ({ reasoningEffort })),
                }];
            });
            if (!models.length) {
                setAgentState({ models: [], reasoningEffort: "" });
                return;
            }
            ...
            const nextEffort = levels.includes(savedEffort as AgentReasoningEffort) ? savedEffort as AgentReasoningEffort : current.defaultReasoningEffort || "";
            localStorage.setItem("canvas-agent-model", current.model);
            if (nextEffort) localStorage.setItem("canvas-agent-reasoning-effort", nextEffort);
            else localStorage.removeItem("canvas-agent-reasoning-effort");
    ...
    const cancelEditUserMessage = () => {
        editingUserMessageRef.current = null;
        setEditingUserMessageId("");
    };

    const beginEditUserMessage = (item: AgentChatItem) => {
        const current = useAgentStore.getState();
        ...
        if (item.role !== "user" || !item.turnId) return;
        const keepThroughTurnId = keepThroughTurnIdBeforeUserMessage(current.messages, item.id);
        const canvasReferences = (item.canvasReferences || []).map((reference) => ({
            id: reference.nodeId,
        ...
        const restoredAttachments = (item.attachments || []).flatMap((attachment) => {
        ...
        restoredAttachments.forEach((attachment) => attachmentUrlsRef.current.add(attachment.url));
        clearSkillSelection();
        if (item.skill) {
            const matched = useAgentSkillStore.getState().skills.find((skill) => skill.name === item.skill!.name && skill.path === item.skill!.path && skill.enabled);
            if (matched) selectSkill(matched, item.text);
            else setAgentState({ prompt: item.text });
        } else {
            setAgentState({ prompt: item.text });
        }
        setAgentState({
            activeTab: "chat",
            canvasReferences,
        ...
        editingUserMessageRef.current = { messageId: item.id, keepThroughTurnId };
        setEditingUserMessageId(item.id);
    };

    const prepareEditRollback = async () => {
        const edit = editingUserMessageRef.current;
        if (!edit) return true;
        const current = useAgentStore.getState();
        const threadId = current.activeThreadId;
        if (!threadId) return false;
        const operation = beginThreadOperation();
        setAgentState({ activity: rt("rollingBack") });
        try {
            if (edit.keepThroughTurnId) {
                const result = await fetchAgentJson<AgentThreadResponse>(endpoint, token, `/agent/codex/threads/${encodeURIComponent(threadId)}/fork`, {
                    method: "POST",
                    headers: { "content-type": "application/json" },
                    body: JSON.stringify({ clientId: clientIdRef.current, permissionMode, lastTurnId: edit.keepThroughTurnId }),
                });
                if (result.conversation) applyConversationState(result.conversation, true);
                const nextThreadId = result.thread?.id || result.conversation?.threadId || "";
                if (!nextThreadId) throw new Error(rt("rollbackFailed"));
                const history = normalizeHistoryMessages(result.messages || []);
                threadMessagesRef.current.set(nextThreadId, history);
                if (result.settledTurnIds?.length) authoritativeHistoryTurnsRef.current = authoritativeHistoryTurnKeys(nextThreadId, result.settledTurnIds);
                else authoritativeHistoryTurnsRef.current.clear();
                liveTurnKeysRef.current.clear();
                setAgentState({
                    activeThreadId: nextThreadId,
                    activeTurnId: "",
                ...
                if (result.conversation) applyConversationState(result.conversation, true);
                setAgentState({ messages: [], tokenUsage: null, pendingTool: null, pendingApprovals: [], activity: rt("rolledBack") });
            }
            ...
            const state = agentErrorState(error);
            if (state) applyConversationState(state);
            addEventLog(rt("rollbackFailed"), error);
            message.error(error instanceof Error ? error.message : rt("rollbackFailed"));
            return false;
        } finally {
    ...
    const sendPrompt = async (staleAttempt = 0) => {
        ...
        if (editingUserMessageRef.current) {
            const rolledBack = await prepareEditRollback();
        ...
        const userText = userTextWithCanvasReferences(text, canvasReferences, rt(files.length ? "imagesSent" : "canvasReferencesSent", { count: files.length || canvasReferences.length }));
        ...
        try {
            ...
            const accepted = await fetchAgentJson<AgentTurnResponse>(endpoint, token, "/agent/codex/turn", {
                ...
                body: JSON.stringify({
                    ...
                    permissionMode,
                    model,
                    thinkingLevel: reasoningEffort || undefined,
    ...
    const stopTurn = async () => {
        ...
        try {
            const threadId = useAgentStore.getState().activeThreadId;
            await interruptCodexTurn(endpoint, token, threadId || undefined);
            flushReasoning();
            const current = useAgentStore.getState();
            setAgentState({
                waiting: false,
                sending: false,
                activeTurnId: "",
                activity: rt("taskStopped"),
                messages: settleStoppedActivities(current.messages, threadId, current.activeTurnId),
            });
    ...
    useEffect(() => {
        if (!usesLocalCanvasAgent() || autoConnectRef.current || enabled || connected) return;
    ...
    const documentSessionNodeId = documentSession?.nodeId || "";
    ...
    const threadSwitchBlocked = () => {
        const current = useAgentStore.getState();
        return !current.connected || current.sending || current.waiting || current.loadingThreads || ["preparing", "running"].includes(current.conversation.status);
    };

    useEffect(() => {
        if (documentSessionNodeId) {
            restorePendingRef.current = "";
            if (documentOpenedForRef.current !== documentSessionNodeId) {
                documentOpenedForRef.current = documentSessionNodeId;
                documentThreadStartedForRef.current = "";
                restoreThreadIdRef.current = documentSession?.previousThreadId || "";
                const current = useAgentStore.getState();
                if (current.activeThreadId && current.messages.length) threadMessagesRef.current.set(current.activeThreadId, current.messages);
                setAgentState({ messages: [], tokenUsage: null, activeTab: "chat" });
            }
            if (documentThreadStartedForRef.current === documentSessionNodeId || threadSwitchBlocked()) return;
            documentThreadStartedForRef.current = documentSessionNodeId;
            void startNewThread();
            return;
        }
        if (documentOpenedForRef.current) {
            documentOpenedForRef.current = "";
            documentThreadStartedForRef.current = "";
            restorePendingRef.current = restoreThreadIdRef.current;
            restoreThreadIdRef.current = "";
        }
        const restoreId = restorePendingRef.current;
        if (!restoreId || threadSwitchBlocked()) return;
        restorePendingRef.current = "";
        void resumeThread(restoreId);
    }, [connected, conversation.status, documentSessionNodeId, loadingThreads, sending, setAgentState, waiting]);
    ...
    const appendActivityDelta = (event: AgentEventPayload) => {
        ...
        if (item.type === "reasoning") {
            ...
            queueReasoning(scoped.id, (activityItems, current) => {
                ...
                return { ...scoped, title: i18n.t("agent.events.reasoning"), text: reasoningActivityText(activityItems), activityItems, detail: activityDetail(current?.detail || scoped.detail, "reasoning", "inProgress") };
            });
            return;
        }
    ...
    const handleAgentEvent = async (event: AgentEventPayload) => {
        ...
        if (event.type === "item.completed" && event.item?.type === "agent_message" && event.item.id) {
            const scoped = scopeEventChatItem(event, { id: event.item.id, role: "assistant", title: "Agent", text: stringText(event.item.text) }, event.item.id);
            const currentMessages = useAgentStore.getState().messages;
            const index = currentMessages.findIndex((message) => message.id === scoped.id);
            if (index >= 0) {
        ...
        if (event.type === "turn.completed") {
            ...
            setAgentState({
                ...
                messages: settleStoppedActivities(current.messages, scope.threadId, scope.turnId),
            });
    ...
    const appendStreamText = (event: AgentEventPayload, text: string, isDelta = false) => {
        ...
        const itemId = event.item?.id;
        if (!itemId) return;
        const scoped = scopeEventChatItem(event, { id: itemId, role: "assistant", title: "Agent", text, streamId: itemId }, itemId);
        const currentMessages = useAgentStore.getState().messages;
        const index = currentMessages.findIndex((message) => message.id === scoped.id);
        if (index < 0) {
            pushMessage(scoped);
    ...
    const content = (
        <div className="flex h-full min-h-0 flex-col">
            <div className="flex h-16 shrink-0 items-center justify-between gap-2 border-b px-3" style={{ borderColor: theme.node.stroke }}>
                <div className="flex min-w-0 items-center gap-2">
                    ...
                    <div className="min-w-0">
                        <div className="truncate text-sm font-semibold leading-5">{documentSession ? t("agent.panel.documentAgent") : "Research Agent"}</div>
                ...
                <div className="flex shrink-0 items-center gap-1">
                    <Tooltip title={t("agent.panel.history")} placement="bottom">
                        <Button type="text" shape="circle" className="!h-8 !w-8 !min-w-8" aria-label={t("agent.panel.historyLabel")} style={{ color: theme.node.muted }} icon={<History className="size-4" />} onClick={() => {
                ...
                <div className="flex min-h-0 flex-1 flex-col">
                    {settingsTab === "setup" ? (
                        <AgentConnectView
                            ...
                            onDisconnect={() => void toggleAgentConnection({ silent: false })}
                            onRetry={() => {
                                clearAgentSession({ enabled: false, connected: false, connectError: "" });
                                void toggleAgentConnection({ silent: false });
                            }}
                        />
                    ) : settingsTab === "skills" ? (
            ...
            {editingUserMessageId ? (
                <div className="flex items-center justify-between gap-2 px-4 pb-1 text-[11px]" style={{ color: theme.node.muted }}>
                    <span>{t("agent.message.editHint")}</span>
                    <button type="button" className="rounded-md px-1.5 py-0.5 transition hover:bg-black/5 dark:hover:bg-white/10" style={{ color: theme.node.text }} onClick={cancelEditUserMessage}>
                        {t("common.cancel")}
                    </button>
                </div>
            ) : null}
            <AgentChatComposer
                ...
                placeholder={conversation.status === "idle" || conversation.status === "preparing"
                    ? t("agent.panel.preparing")
                    : conversation.status === "failed"
                        ? t("agent.panel.initFailed")
                        : !models.length
                            ? t("agent.panel.needApiKey")
                    : t(documentSession ? "agent.panel.documentPlaceholder" : "agent.panel.placeholder")}
                theme={theme}
                ...
                onModelChange={(model) => {
                    ...
                    if (!selected) return;
                    const effort = selected.defaultThinkingLevel || selected.defaultReasoningEffort || selected.thinkingLevels?.[0] || selected.supportedReasoningEfforts[0]?.reasoningEffort || "";
