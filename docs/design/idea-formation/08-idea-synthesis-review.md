# CoResearch Step 8–10：Idea Draft / Review / Versioning

- 日期：2026-09-18
- 状态：设计提案；本次不实现 Review Agent、Canvas 命令、文献检索或 Idea 导出。
- 上游：[Step 7 · Research Design Co-Design](./07-research-design.md)。
- 画布：[Huabu 领域映射与交互升级提案](../canvas/huabu-domain-binding.md)。
- 产物：可继续演化的 `Idea Draft`、`Idea Snapshot` milestone，以及 Section-level revision。
...
Idea 不再是一次生成后固定的长文档，而是由 Problem、Research Question、Core Claim、Method、Evaluation、Evidence / Related Work、Limitation / Risk 和 Contribution 等可替换 Section 组成。每个 Section 独立维护 Current、Evidence、Proposals、History，并可单独 Ask、Prior Art Search、Challenge 和 Replace。Section 替换后只触发 dependency check，不自动级联修改其它 Section；确认相关依赖后才形成新的 semantic Idea revision。

## 1. Idea Draft / Review 的目标

**把已确认的 Research Design Candidate 收束成一个可继续演化的 Idea Draft，并检查各个 Idea Section 与 Literature Position 是否彼此对齐。**

Step 8 不是一次新的 brainstorming，也不是把所有内容重新交给模型改写成一篇 proposal。它只做四件事：

```text
收束       从 Research Design Candidate 形成 Idea Draft
对齐       检查 Problem → RQ → Claim → Method → Evaluation
局部审查   对单个 Idea Section 做 Ask / Search / Challenge
确认       由用户确认 Section 组合，建立 Idea Snapshot milestone
```

完整流程到此收束：

```text
Seed → Direction Exploration → Direction Polish → Selected Direction
   ↓
Deep Dive → Paper Sections → Research Question
   ↓
Evaluation Exploration → Method Exploration
   ↓
Research Design Co-Design → Idea Draft → Section-level Review
```
...
interface IdeaSynthesisRequest {
  ...
  directionRef: VersionedRef
  researchQuestionRef: VersionedRef
  evaluationStrategyRef: VersionedRef
